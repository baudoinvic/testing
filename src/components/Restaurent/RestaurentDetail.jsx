          ...
          <div className="bg-blue-50 rounded-lg p-4 sm:p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto relative">
            ...
            {buttonTwo.sections.map((section, sectionIndex) => (
              <div key={sectionIndex}>
                <h2 className="text-xl font-bold mb-4 sm:mb-6 pr-8">{section.title}</h2>
                
                <div className="space-y-4 mb-6 sm:mb-8">
                  {section.items && section.items.map((item, itemIndex) => (
                    <div key={itemIndex} className="bg-white rounded-lg p-3 sm:p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                      <div className="font-medium">{item.name}</div>
                      <div className="text-gray-600 text-sm sm:text-base">{item.days}</div>
